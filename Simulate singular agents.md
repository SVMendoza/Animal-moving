# Animal-moving
Algorithm for dispersal seed by monkey (Trial and development)
...
for(j in 1:perm) ##número de ciclos
  ...
  Tres<-c()
              ...
              Tca<-sample(as.vector(Tcam), 1,replace =T)
              Dis<-Vt*Tca
              X1<-Xini+cos(A)*Dis
              Y1<-Yini+sin(A)*Dis
              Td<-Tca
     ...
     while(Per)
        {
                  ...
                  TablaXY[i,2]<-X1
                  TablaXY[i,3]<-Y1
                  Tc<-sample(as.vector(Tcom), 1, replace =T)
                  Td<-Td+Tc
                  Per1<-Per1+Tc
                                            ...
                                            if(is.null(l1))
                                               ...
                                               TDig<-sample(as.vector(Tdig),1,replace =T)
                                               Tpar<-Td+TDig
                                               index<-i
                                               TablaXY[i,1]<-index
                                               TablaXY[i,2]<-X1
